...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 flex items-center justify-between px-4 md:px-6 py-3 bg-[#0a0a0a]/80 backdrop-blur-md border-b border-white/5">
      ...
      {/* Navigation gauche — boutons précédent/suivant (desktop) */}
      <div className="flex items-center gap-2">
        <button
          onClick={() => navigate(-1)}
          className="hidden md:flex items-center justify-center w-8 h-8 rounded-full bg-black/50 text-white/70 hover:text-white hover:bg-white/10 transition-all"
          aria-label="Retour"
        >
          <ChevronLeft />
        </button>
        ...
        {/* Titre page (mobile) */}
        <span className="md:hidden text-white font-bold text-lg tracking-tight">
          {getPageTitle()}
        </span>
      </div>

      {/* Logo centré (desktop uniquement) */}
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-2 bg-black/50 hover:bg-white/10 transition-all px-3 py-1.5 rounded-full text-white/80 hover:text-white"
        >
